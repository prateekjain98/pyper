...
!define SHERPA_FIREWALL_RULE "Pyper Local Transcription Server (sherpa-onnx)"
...
!macro customUnInstall
  ...
  ${ifNot} ${isUpdated}
    !insertmacro DeleteAutoStartEntry "com.saaslabs.pyper"
    !insertmacro DeleteAutoStartEntry "com.saaslabs.pyper.staging"
    !insertmacro DeleteAutoStartEntry "com.saaslabs.pyper.development"
    nsExec::ExecToLog 'netsh advfirewall firewall delete rule name="${SHERPA_FIREWALL_RULE}"'
    StrCpy $0 "$PROFILE\.cache\pyper\models"
    IfFileExists "$0\*.*" 0 +3
      RMDir /r "$0"
      DetailPrint "Removed Pyper cached models"
    StrCpy $1 "$PROFILE\.cache\pyper"
    RMDir "$1"
  ${endIf}
!macroend
