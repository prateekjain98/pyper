...
Outbound hosts the Pyper desktop app contacts. For firewall, proxy, and
...
Contacted by every install using Pyper Cloud (the default after
onboarding).

| Host                                          | Protocol | Port | Purpose                                                                            |
| --------------------------------------------- | -------- | ---- | ---------------------------------------------------------------------------------- |
| `api.pyper.work`                          | HTTPS    | 443  | Cloud API: transcription, sync, agent reasoning, settings, usage.                  |
| `auth.pyper.work`                         | HTTPS    | 443  | Account sign-in and session refresh (Better Auth).                                 |
...
Pyper Cloud routes streaming sessions through one of three providers.
...
| `pyper.work`        | HTTPS    | 443  | OAuth desktop callback redirect (`/auth/desktop-callback`). |
...
```sh
# Pyper Cloud reachability
curl -v https://api.pyper.work/api/health
