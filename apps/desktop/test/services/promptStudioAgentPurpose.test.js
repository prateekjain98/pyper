...
test("Prompt Studio labels dictation-agent runs for policy enforcement", async (t) => {
  ...
  const cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), "pyper-prompt-studio-test-"));
            ...
            return `
              export function resolveDictationAgentInference() {
                return {
                  reachable: true,
                  model: "auto",
                  displayProvider: "pyper",
                  config: { provider: "pyper" },
                };
              }
            `;
          }
