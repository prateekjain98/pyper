...
const ICON_THEME_SUBPATH = path.join("icons", "hicolor", "256x256", "apps", "pyper.png");
...
function withTmpXdgDirs(fn) {
  return async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pyper-autostart-test-"));
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/home/user/Applications/Pyper.AppImage";
    assert.equal(resolveExecutablePath(), "/home/user/Applications/Pyper.AppImage");
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    fs.writeFileSync(path.join(installDir, "pyper"), "#!/bin/bash\n");
    withExecPath(path.join(installDir, "pyper-app"), () => {
      assert.equal(resolveExecutablePath(), path.join(installDir, "pyper"));
    });
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    const orphan = path.join(root, "opt", "pyper-app");
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents("/a/b/Pyper.AppImage", "pyper");
    assert.match(contents, /^Exec="\/a\/b\/Pyper\.AppImage" --hidden$/m);
    assert.match(contents, /^Icon=pyper$/m);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/apps/Pyper.AppImage";
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents("/a/b/Pyper.AppImage", null);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents('/home/o"neill/$HOME/`app`\\v/Pyper', null);
    assert.match(
      contents,
      /^Exec="\/home\/o\\"neill\/\\\$HOME\/\\`app\\`\\\\\\\\v\/Pyper" --hidden$/m
    );
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    setAutostartEnabled(true);
    assert.match(fs.readFileSync(getDesktopFilePath(), "utf8"), /^Icon=pyper$/m);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/old/path/Pyper.AppImage";
    setAutostartEnabled(true);

    process.env.APPIMAGE = "/new/path/Pyper-1.9.0.AppImage";
    assert.equal(syncAutostartEntry(), true);

    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/new\/path\/Pyper-1\.9\.0\.AppImage" --hidden$/m);
  })
);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/path/Pyper.AppImage";
    setAutostartEnabled(true);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/new/path/Pyper.AppImage";
    writeEntry(
      getDesktopFilePath(),
      '[Desktop Entry]\nType=Application\nExec="/old/path/Pyper.AppImage"\nX-GNOME-Autostart-enabled=false\n'
    );
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/installed/Pyper.AppImage";
    setAutostartEnabled(true);
    ...
    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/installed\/Pyper\.AppImage" --hidden$/m);
  })
);
