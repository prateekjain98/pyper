---
name: pyper-cli
description: Use this skill whenever the user wants to operate on Pyper notes, folders, transcriptions, or audio from a terminal or shell. The Pyper CLI (`pyper` binary, npm package `@pyper/cli`) talks to either the local desktop app or the cloud API and exposes every operation needed for managing notes, folders, transcriptions, audio, plus auth and config. Trigger this skill when the user mentions "pyper cli", running shell commands against Pyper, automating note workflows, cleaning up a transcription, building agent integrations against Pyper, or scripting any Pyper operation — even if they don't say "CLI" explicitly.
---

# Pyper CLI

Use this reference when running the `pyper` command-line tool. The CLI is a single binary that operates against either the local desktop app (via a loopback HTTP bridge) or the cloud REST API. The same command works against both backends.

## Install

```bash
npm install -g @pyper/cli
```

Requires Node.js 20 or later. Verify with `pyper --version`. If the user reports `command not found`, ensure their npm global bin is on `$PATH`.
...
| **remote** | `https://api.pyper.work/api/v1`               | Desktop is closed, or running on a different machine, or the user wants cloud-side semantics. |
...
2. The `PYPER_BACKEND` environment variable (`local`, `remote`, or `auto`)
3. The `backend` key in `~/.pyper/cli-config.json`
...
When the desktop app starts, it writes `{version, port, token}` to `~/.pyper/cli-bridge.json` with mode `0600`. The CLI reads it automatically. If the file is missing or stale, local is treated as unavailable.
...
```bash
pyper auth login    # prompts for the key, stores it 0600 in ~/.pyper/cli-config.json
pyper auth status   # confirm it works
pyper auth logout   # clear it
```
...
Noun-verb syntax: `pyper <noun> <verb>`. Same convention as `gh`, `kubectl`, `aws`, `stripe`.

### Notes

```bash
pyper notes list [--folder <id>] [--limit N] [--format json|table]
pyper notes get <id> [--format json|markdown]
pyper notes create --content <text> | --content-file <path>
                        [--title <t>] [--folder <id>]
pyper notes update <id> [--content <t>] [--folder <id>] [--title <t>]
pyper notes delete <id> [--dry-run] [--format json]
pyper notes search <query> [--limit N] [--format json|table]
```

### Folders

```bash
pyper folders list [--format json|table]
pyper folders create --name <name> [--sort-order <n>]
```
...
```bash
pyper transcriptions list [--limit N] [--format json|table]
pyper transcriptions get <id> [--format json|text]
pyper transcriptions delete <id> [--dry-run] [--format json]
```
...
```bash
pyper audio delete <transcription-id> [--format json]
```
...
```bash
pyper auth login [--api-key <key>]   # prompts on stdin if --api-key omitted
pyper auth status [--format json]
pyper auth logout
```

`auth status` reads the stored config and reports whether a key is configured — it does **not** make a network call. To verify the key actually works, use `pyper doctor`.

### Config

```bash
pyper config get [--format json]
pyper config set backend auto|local|remote
pyper config set api-base https://api.pyper.work
```

Only `backend` and `api-base` are settable via `config set`. The API key is managed through `auth login`/`auth logout`. `api-base` is overridable for self-hosted or staging deployments (default: production cloud). The `PYPER_API_BASE` env var also overrides it for a single invocation.

### Doctor

```bash
pyper doctor [--format json]
```
...
```bash
pyper --version    # or: pyper version
```
...
```bash
pyper notes list --limit 100 --format json | \
  jq -r '.[] | select(.title | contains("draft")) | .id' | \
  while read id; do
    pyper notes delete "$id"
  done
```
...
```bash
pyper notes search "quarterly budget" --format json | jq '.[].id'
```
...
| ------------------------------- | --------------------------------------- | ------------------------------------------------ |
| `~/.pyper/cli-bridge.json` | The desktop app at startup              | `{version, port, token}` for the loopback bridge |
| `~/.pyper/cli-config.json` | The CLI's `auth login` and `config set` | `{backend, apiBase, apiKey}`                     |
...
| ----------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------- |
| `Backend unreachable` (exit 2) on every command | Desktop closed and no API key                                 | Start desktop, or `pyper auth login`          |
...
| Config file readable by other users             | File created or edited outside the CLI                        | `chmod 0600 ~/.pyper/cli-config.json`         |
