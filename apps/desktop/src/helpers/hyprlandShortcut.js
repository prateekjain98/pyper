...
const BINDS_FILENAME = "pyper-binds.conf";
const MANAGED_HEADER_LINES = [
  "# Pyper keybinds (managed automatically)",
...
class HyprlandShortcutManager {
   ...
   *
   * Also writes the bind to pyper-binds.conf (sourced from hyprland.conf)
  ...
  async registerKeybinding(hotkey) {
    ...
    try {
      // First unregister any existing Pyper binding if the hotkey changed.
