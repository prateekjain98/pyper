// Gmail OAuth for meeting detection. Deliberately a SEPARATE helper from
// googleCalendarOAuth.js: it requests the read-only Gmail scope and stores its
// tokens in the `gmail_tokens` store, so connecting Gmail never re-prompts or
// disturbs the calendar grant (and vice-versa).
//
// It reuses the same Google OAuth client credentials as calendar
// (GOOGLE_CALENDAR_CLIENT_ID / _SECRET) to keep setup to one Cloud project — the
// Gmail API must be enabled on that project for the grant to work.
...
class GmailOAuth {
      ...
      handleCallback: async (code, redirectUri, codeVerifier) => {
        ...
        if (tokenData.id_token) {
          try {
            ...
            /* fall through to the no-email guard */
          }
        }
        if (!email) {
          throw new OAuthFlowError("no_email", "Could not extract email from Gmail OAuth response");
        }
  ...
  async revokeToken(token) {
    ...
    try {
      ...
      // Best-effort — token may already be revoked or the network is down.
    }
  }
