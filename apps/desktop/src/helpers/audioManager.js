...
`;
  ...
  async processAgentCommand(text, model, agentName, config) {
    ...
    const completionMarker = `__PYPER_SELECTION_COMPLETE_${crypto.randomUUID()}__`;
  ...
  async runTranslationChain({ text, settings, agentName, route, cleanup }) {
    ...
    try {
      const chainResult = await executeTranslationChain({
        ...
        translateIsCloud: route.config?.provider === "pyper",
  ...
  async processWithPyperCloud(audioBlob, metadata = {}) {
    if (!navigator.onLine) {
    ...
    const cleanupCloudMode = settings.cleanupCloudMode || "pyper";
    if (
      (settings.useCleanupModel && !this.skipReasoning && cleanupCloudMode === "pyper") ||
      (this.translationRequested &&
        !this.skipReasoning &&
    ...
    if (processedText && !this.skipReasoning) {
      ...
      try {
        ...
        } else if (route.kind === "cleanup" && cleanupCloudMode === "pyper") {
          const reasonResult = await withSessionRefresh(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) err.code = "INVALID_KEY";
          ...
          // The user's own provider rate-limited the request — not an Pyper plan limit
  ...
  shouldUseStreaming(isSignedInOverride) {
    ...
    // Corti (BYOK) streams over its own WSS — independent of Pyper Cloud.
    if (s.cloudTranscriptionProvider === "corti" && s.cloudTranscriptionMode === "byok") {
      return !!(s.cortiClientId && s.cortiClientSecret);
    }

    // Tinfoil realtime streams without an Pyper account.
    if (s.cloudTranscriptionProvider === "tinfoil") {
    ...
    if (s.cloudTranscriptionMode !== "pyper" || !(isSignedInOverride ?? s.isSignedIn)) {
      return false;
    }
      ...
      const [, wsResult] = await Promise.all([
        ...
        withSessionRefresh(async () => {
          ...
          const res = await provider.warmup({
            ...
            mode: cloudTranscriptionMode === "byok" ? "byok" : "pyper",
  ...
  async startStreamingRecording(forceDefaultMic = false) {
    ...
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your Pyper Cloud session is unavailable. Please sign in again from Settings.";
  ...
  async stopStreamingRecording() {
    ...
    if (finalText && !this.skipReasoning) {
      ...
      const cleanupCloudMode = stSettings.cleanupCloudMode || "pyper";
    ...
    if (!finalText && durationSeconds > 2 && fallbackBlob?.size > 0) {
      ...
      if (target === "skip") {
        logger.warn(
          "Skipping batch fallback: Pyper Cloud session signed out",
          {},
          "streaming"
        );
      } else {
        ...
        try {
          ...
          const batchResult =
            target === "cloud"
              ? await this.processWithPyperCloud(fallbackBlob, { durationSeconds })
