...
inputs.pyper.url = "github:Pyper/pyper";

# in your NixOS modules
imports = [ pyper.nixosModules.default ];
programs.pyper = {
  enable = true;
  users = [ "<you>" ];
};`;
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {t("settingsPage.general.waylandPaste.nixos.flakeDesc", {
            defaultValue:
              "If you install Pyper from the flake, programs.pyper.enable turns on ydotool, the uinput module and the group memberships for you.",
          })}
        </p>
