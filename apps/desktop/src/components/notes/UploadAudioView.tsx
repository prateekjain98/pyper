...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  const isPyperCloud =
    isSignedIn && cloudTranscriptionMode === "pyper" && !useLocalWhisper;

  // Mode detection
  const isSelfHosted = transcriptionMode === "self-hosted" && !useLocalWhisper;
  const isByok = !useLocalWhisper && !isPyperCloud;
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      // Cloud (Pyper) — user is always signed in here
  ...
  const getActiveModelLabel = (): string => {
    if (isPyperCloud) return t("notes.upload.pyperCloud");
    if (useLocalWhisper) {
      if (localTranscriptionProvider === "nvidia")
  ...
  const handleTranscribe = async () => {
    ...
    const useChunkProgress = isPyperCloud && isLargeFile;
  ...
  const switchToCloud = () => {
    setUploadTranscriptionMode("pyper");
    setUploadCloudTranscriptionMode("pyper");
    setUploadUseLocalWhisper(false);
  };

  const getTranscribingLabel = (): string => {
    if (isPyperCloud) return t("notes.upload.transcribingCloud");
      ...
      >
        ...
        {(state === "idle" || state === "selected") && (
          <div className="max-w-[320px] mx-auto mt-4">
            ...
            {diarizationEnabled && isPyperCloud && (
              <p className="text-[10px] text-foreground/25 mt-1.5">
...
function SelectedView({
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {isLargeFile && !requiresUpgrade && !fileTooLarge && isPyperCloud && (
