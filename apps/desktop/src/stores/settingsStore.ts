...
const TRANSCRIPTION_POLICY_CATALOG = {
  modes: ["pyper", "providers", "local", "self-hosted"] as const,
...
const MEETING_TRANSCRIPTION_POLICY_CATALOG = {
  ...
  modes: ["pyper", "providers", "local"] as const,
...
const LLM_POLICY_CATALOG = {
  modes: ["pyper", "providers", "local", "self-hosted", "enterprise"] as const,
...
function migrateProviderSettings() {
  ...
  let newReasoningMode: InferenceMode = "pyper";
...
// so nothing is copied and the upload context falls through to its Pyper
// Cloud defaults.
...
function migrateAgentMode() {
  ...
  let agentInferenceMode: InferenceMode = "pyper";
...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  cloudTranscriptionMode: readString("cloudTranscriptionMode", "pyper"),
  cleanupCloudMode: readString("cleanupCloudMode", "pyper"),
  ...
  transcriptionMode: (() => {
    const v = readString("transcriptionMode", "pyper");
    if (v === "pyper" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "pyper" as InferenceMode;
  })(),
  ...
  cleanupMode: (() => {
    const v = readString("cleanupMode", "pyper");
    if (
      v === "pyper" ||
      v === "providers" ||
      v === "local" ||
  ...
  meetingTranscriptionMode: (() => {
    const v = readString("meetingTranscriptionMode", "pyper");
    if (v === "pyper" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "pyper" as InferenceMode;
  })(),
  ...
  uploadTranscriptionMode: (() => {
    const v = readString("uploadTranscriptionMode", "pyper");
    if (v === "pyper" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "pyper" as InferenceMode;
  })(),
  ...
  noteFormattingMode: (() => {
    const v = readString("noteFormattingMode", "pyper");
    if (
      v === "pyper" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationMode: (() => {
    const v = readString("translationMode", "pyper");
    if (
      v === "pyper" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationCloudMode: readString("translationCloudMode", "pyper"),
  ...
  chatAgentCloudMode: readString("chatAgentCloudMode", "pyper"),
  chatAgentMode: (() => {
    const v = readString("chatAgentMode", "pyper");
    if (
      v === "pyper" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentMode: (() => {
    const v = readString("dictationAgentMode", "pyper");
    if (
      v === "pyper" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentCloudMode: readString("dictationAgentCloudMode", "pyper"),
  ...
  dictationAgentVisionMode: (() => {
    const v = readString("dictationAgentVisionMode", "pyper");
    if (v === "pyper" || v === "providers") return v as InferenceMode;
    return "pyper" as InferenceMode;
  })(),
  ...
  dictationAgentVisionCloudMode: readString("dictationAgentVisionCloudMode", "pyper"),
  ...
  setCloudTranscriptionForAllScopes: (settings: Partial<TranscriptionSettings>) => {
    ...
    // scope — otherwise the UI keeps showing the previous mode (e.g. Pyper
...
export const selectIsCloudCleanupMode = (state: SettingsState) =>
  state.isSignedIn && state.cleanupMode === "pyper" && state.cleanupCloudMode === "pyper";

export const selectEffectiveCleanupProvider = (state: SettingsState) =>
  selectIsCloudCleanupMode(state) ? "pyper" : state.cleanupProvider;

export const selectIsCloudChatAgentMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.chatAgentMode === "pyper" &&
  state.chatAgentCloudMode === "pyper";

export const selectIsCloudDictationAgentMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.dictationAgentMode === "pyper" &&
  state.dictationAgentCloudMode === "pyper";

export const selectIsCloudTranslationMode = (state: SettingsState) =>
  state.isSignedIn &&
  state.translationMode === "pyper" &&
  state.translationCloudMode === "pyper";

export const selectIsCloudNoteFormattingMode = (state: SettingsState) => {
  const cfg = selectResolvedNoteFormatting(state);
  return state.isSignedIn && cfg.mode === "pyper" && cfg.cloudMode === "pyper";
};
...
export function selectPolicyEffectiveSettings(
  ...
  for (const keys of TRANSCRIPTION_CONTEXT_KEYS) {
    ...
    writable[keys.cloudMode] = selection.mode === "pyper" ? "pyper" : "byok";
  ...
  for (const scope of Object.keys(INFERENCE_SCOPES) as InferenceScope[]) {
    ...
    if (definition.storeKeys.cloudMode) {
      writable[definition.storeKeys.cloudMode] =
        selection.mode === "pyper" ? "pyper" : "byok";
    }

    let provider = selection.provider;
    if (selection.mode === "pyper") provider = "pyper";
      ...
      if (!localStorage.getItem("enterpriseSetupMode")) {
        ...
        const hasChosenProvider =
          Object.values(INFERENCE_SCOPES).some((scope) => {
            ...
            return Boolean(stored) && stored !== "pyper";
          }) ||
          Boolean(
