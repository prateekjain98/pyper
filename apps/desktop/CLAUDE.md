# Pyper Technical Reference for AI Assistants

This document provides comprehensive technical details about the Pyper project architecture for AI assistants working on the codebase.

## Project Overview

Pyper is an Electron-based desktop dictation application that uses whisper.cpp for speech-to-text transcription. It supports both local (privacy-focused) and cloud (OpenAI API) processing modes.
...
- **hyprlandShortcut.js**: Hyprland Wayland global shortcut integration
  - Uses D-Bus service to receive hotkey toggle commands (same `com.saaslabs.pyper.App` service)
...
- **linuxAutostart.js**: Launch-at-login on Linux via an XDG autostart entry
  - `app.setLoginItemSettings()` is a no-op on Linux, so the entry is written directly to `$XDG_CONFIG_HOME/autostart/pyper.desktop`, matching the executable name electron-builder packages under
...
- **cliBridge.js**: Loopback HTTP server on ports 8200–8219, bearer-token auth (token at `~/.pyper/cli-bridge.json`), 127.0.0.1-only. Used by the unified CLI to talk to a running desktop app.
...
- **ReasoningService.ts**: AI processing for agent-addressed commands
  ...
  - Provider implementations live in a registry at `src/services/ai/inferenceProviders/index.ts` covering 8 providers (`anthropic`, `enterprise`, `gemini`, `groq`, `lan`, `local`, `openai`, `pyper`), each implementing the `InferenceProvider` interface from `types.ts`
...
- **whisper.js**: Native binary wrapper for local transcription
  ...
  - Models stored in `~/.cache/pyper/whisper-models/`
...
- **parakeet.js**: Model management for NVIDIA Parakeet ASR models
  ...
  - Models stored in `~/.cache/pyper/parakeet-models/`
...
- Qdrant data: `~/.cache/pyper/qdrant-data/` (`qdrant-data-dev/` in development)
- Qdrant binary: `resources/bin/qdrant-{platform}-{arch}` (bundled — downloaded during `prebuild` / `predev:main`)
- Embedding model: `~/.cache/pyper/embedding-models/all-MiniLM-L6-v2/` (auto-downloaded on first launch)
...
Enable with `--log-level=debug` or `PYPER_LOG_LEVEL=debug` (can be set in `.env`):
...
On GNOME Wayland, Electron's `globalShortcut` API doesn't work due to Wayland's security model. Pyper uses native GNOME shortcuts:
...
3. `gnomeShortcut.js` creates D-Bus service at `com.saaslabs.pyper.App`
...
- D-Bus service: `com.saaslabs.pyper.App`
- D-Bus path: `/com/pyper/App`
- gsettings path: `/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/pyper/`
...
On Hyprland (wlroots Wayland compositor), Electron's `globalShortcut` API and the `GlobalShortcutsPortal` feature don't work reliably. Pyper uses native Hyprland keybindings:
...
3. `hyprlandShortcut.js` creates D-Bus service at `com.saaslabs.pyper.App` (same as GNOME)
...
- Image-wired clients: `openai` (Responses `input_image` / Chat `image_url`, also `custom`/`openrouter`), `anthropic` (base64 content block via `process-anthropic-reasoning`), `gemini` (`inlineData`), `pyper` (forwards `screenContext` + `promptMode: "agent"` to `/api/reason`; the server routes to its `REASONING_VISION_*` model chain)
...
4. Do NOT translate: brand names (Pyper, Pro), technical terms (Markdown, Signal ID), format names (MP3, WAV), AI system prompts
...
7. **Local Semantic Search Not Working**:
   ...
   - Embedding model should be in `~/.cache/pyper/embedding-models/all-MiniLM-L6-v2/model.onnx` (auto-downloaded on first app launch)
...
- **Launch at login**: XDG autostart entry at `~/.config/autostart/pyper.desktop` (see `linuxAutostart.js`), since Electron's `setLoginItemSettings()` does nothing on Linux
