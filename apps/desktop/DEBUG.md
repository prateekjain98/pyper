...
```bash
# macOS
/Applications/Pyper.app/Contents/MacOS/Pyper --log-level=debug

# Windows
Pyper.exe --log-level=debug
```
...
```
PYPER_LOG_LEVEL=debug
```

**Env file locations:**

- macOS: `~/Library/Application Support/Pyper/.env`
- Windows: `%APPDATA%\Pyper\.env`
- Linux: `~/.config/Pyper/.env`

## Log File Locations

- **macOS**: `~/Library/Application Support/Pyper/logs/debug-*.log`
- **Windows**: `%APPDATA%\Pyper\logs\debug-*.log`
- **Linux**: `~/.config/Pyper/logs/debug-*.log`
...
- Remove `PYPER_LOG_LEVEL` from `.env`
