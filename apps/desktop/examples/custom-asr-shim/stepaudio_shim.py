#!/usr/bin/env python3
"""Local shim that bridges Pyper's Self-Hosted transcription to
StepFun's StepAudio 2.5 ASR API.

Pyper POSTs OpenAI-style multipart/form-data and expects JSON {"text": ...}.
...
Then in Pyper: Settings -> Transcription -> Self-Hosted,
...
def transcribe(audio_path: str, model: str, language: str | None, prompt: str | None) -> str:
    ...
    `model`/`language` from Pyper are available but StepAudio is pinned to
