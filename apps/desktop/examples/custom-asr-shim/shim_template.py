#!/usr/bin/env python3
"""Generic local shim that lets Pyper's Self-Hosted transcription talk
to an ASR backend that does NOT speak the OpenAI /audio/transcriptions
protocol.

The contract Pyper speaks (verified against its source):
  ...
  - Do not require Authorization from Pyper. Hold vendor keys in env vars.
...
def transcribe(audio_path: str, model: str, language: str | None, prompt: str | None) -> str:
    ...
    `model`/`language`/`prompt` come straight from Pyper's UI and may be
...
class ShimHandler(BaseHTTPRequestHandler):
    """Handles the single POST endpoint Pyper calls."""
...
def main() -> None:
    ...
    print("Point Pyper at it: Settings -> Transcription -> Self-Hosted")
