...
const DEFAULT_OAUTH_PROTOCOL_BY_CHANNEL = {
  ...
  staging: "pyper-staging",
  production: "pyper",
};
const BASE_WINDOWS_APP_ID = "com.saaslabs.pyper";
...
function resolveAppChannel() {
  const rawChannel = (process.env.PYPER_CHANNEL || process.env.VITE_PYPER_CHANNEL || "")
    .trim()
    .toLowerCase();
...
process.env.PYPER_CHANNEL = APP_CHANNEL;
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `Pyper-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("pyper.desktop");
}
...
function getOAuthProtocol() {
  const fromEnv = (process.env.VITE_PYPER_PROTOCOL || process.env.PYPER_PROTOCOL || "")
    .trim()
    .toLowerCase();
...
// True source of truth for whether pyper:// resolves on Linux — the same
...
function registerPyperProtocol() {
...
const protocolRegistered = registerPyperProtocol() || isOAuthSchemeRegistered();
...
if (process.platform === "darwin" && app.getName() !== "Pyper") {
  app.setName("Pyper");
}
...
function parseAuthBridgePort() {
  const raw = (process.env.PYPER_AUTH_BRIDGE_PORT || "").trim();
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>Pyper sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
...
async function startApp() {
  ...
  applyPyperOriginHeader(session.defaultSession);
