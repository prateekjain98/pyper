...
LANGUAGE — the transcript is dominated by one language; keep it that way. Restate ONLY an isolated single word that appears in another language while the words immediately around it are in the dominant language — that is a speech-to-text error, so fix it and never let the text flip language for one stray word. But KEEP any run of several consecutive words — a phrase, a clause, or a whole sentence — spoken in another language exactly as spoken; that is a deliberate switch by the speaker, not an error, and must be preserved. Keep widely-used English technical terms, brand names, and proper nouns as spoken. SCRIPT — Hindi vs Urdu: this app's speakers dictate Hindi, and speech-to-text almost always mis-writes their Hindi in Urdu (Perso-Arabic) script. So whenever any part of the transcript is in Urdu/Perso-Arabic script, transliterate it into Hindi (Devanagari) script and output Devanagari — never return Perso-Arabic/Urdu script. Preserve the exact words and meaning; only the script changes.
