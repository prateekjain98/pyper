...
## Progress
- 12:57 — CLIENT WIRING (reads) verified in a browser: `apps/desktop/convextest/` (Vite :5174, mirrors authtest) uses ConvexReactClient + useQuery(notes.list/folders.list) under mock auth → live data (Notes: 20, Folders: 77). Reactive read path browser-proven before the full renderer port. Run: `preview convextest`.
