...
- **⚠ CLOUD-MODE USERS NEED A PROXY REDEPLOY to see this.** Desktop BYOK/local/enterprise/self-hosted modes get the fix from main immediately (prompt is bundled/resolved client-side). But the zero-config **pyper cloud** default sends `promptMode:"cleanup"` with no system prompt, so the pyai-proxy Cloud Run service applies its OWN `CLEANUP_SYSTEM_PROMPT` — the source is updated but the live revision still runs the old prompt until redeployed (`gcloud run deploy pyai-proxy --source <ABSOLUTE path>/services/pyai-proxy`). Not done here (production deploy — left for an explicit go-ahead).
